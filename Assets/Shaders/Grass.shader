Shader "Roystan/Grass"
{
	...
	void geom(triangle vertexOutput IN[3] : SV_POSITION, inout TriangleStream<geometryOutput> triStream)
	{
		...
		[loop]for(int i = 0; i < BLADE_SEGMENTS; i++)
		{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {	
				...
				float4 lightIntensity = NdotL * shadow * _LightColor0 + float4(ambient, 1);

				float4 color = lerp(_BottomColor, _TopColor * lightIntensity, i.uv.y);
